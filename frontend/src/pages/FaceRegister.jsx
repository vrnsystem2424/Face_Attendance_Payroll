...
import Webcam from 'react-webcam';
import * as faceapi from '@vladmandic/face-api';
import { registerFace } from '../redux/slices/faceSlice';
import { updateUser, fetchCurrentUser } from '../redux/slices/authSlice';
import { getBestCameraConfig, enhanceCameraStream } from '../utils/cameraConfig';
...
const FaceRegister = () => {
  ...
  const navigate = useNavigate();
  const webcamRef = useRef(null);
  const { error } = useSelector((s) => s.faces);
  const { user } = useSelector((s) => s.auth);
  ...
  const startCapture = useCallback(() => {
    ...
    setPhase('capturing');
    setStatusMsg(CAPTURE_STEPS[0].instruction);
  }, []);
  ...
  const captureFrame = useCallback(async () => {
    ...
    try {
      ...
      if (detection.detection.score < MIN_FACE_SCORE) {
        setStatusMsg('Face not clear — improve lighting');
        return;
      }
      ...
      if (nextStep >= REQUIRED_FRAMES) {
        setPhase('saving');
        setStatusMsg('All captured! Saving to server...');
        await saveEncodings(newEncodings);
      } else {
        setCurrentStep(nextStep);
        setStatusMsg(CAPTURE_STEPS[nextStep].instruction);
      }
    } catch {
  ...
  const saveEncodings = async (encodings) => {
    try {
      ...
      for (const enc of encodings) {
        for (let i = 0; i < len; i++) averaged[i] += enc[i];
      }
      for (let i = 0; i < len; i++) averaged[i] /= encodings.length;

      // ── Token save karo pehle ──
      const currentToken = localStorage.getItem('token');
      console.log('🔑 Token before face register:', currentToken ? '✅' : '❌');
      ...
      // ── Token check after API call ──
      if (!localStorage.getItem('token') && currentToken) {
        console.log('⚠️ Token clear hua — re-saving');
      ...
      if (result.meta.requestStatus === 'fulfilled') {
        ...
        // ✅ Step 1 — Local Redux update (fast)
        dispatch(updateUser({
          face_registered: true,
          face_encoding: averaged,
          all_encodings: encodings,
          face_capture_count: encodings.length,
        }));

        // ✅ Step 2 — Backend se fresh user fetch karo
        // Yeh ensure karta hai face_encoding sahi se aaye
        const freshUser = await dispatch(fetchCurrentUser());
        console.log('✅ Fresh user fetched:', freshUser.payload?.face_registered);
        console.log('✅ face_encoding length:', freshUser.payload?.face_encoding?.length);
        ...
        setPhase('error');
        setStatusMsg(result.payload || 'Save failed — try again');
      }
    } catch (err) {
      console.error('saveEncodings error:', err);
      setPhase('error');
  ...
  const resetCapture = () => {
    ...
    setCountdown(null);
    setPhase('idle');
    setStatusMsg('Ready to start again');
    if (countdownRef.current) clearInterval(countdownRef.current);
  };
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-4 py-10">
        <div className="w-full max-w-[520px] overflow-hidden rounded-[28px] bg-white shadow-[0_20px_70px_-10px_rgba(26,26,46,0.12)]">
          ...
          <div className="p-6">
            ...
            {/* Status */}
            <div className="mb-5 rounded-xl px-4 py-3 text-center"
              style={{ background: ps.bg, border: `1px solid ${ps.border}20` }}>
              <p className="text-sm font-semibold" style={{ color: ps.accent }}>{statusMsg}</p>
            </div>
            ...
            {cameraConfig ? (
              <div className="relative mb-5 overflow-hidden rounded-2xl bg-[#1A1A2E]">
                ...
                <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                  <div className="h-64 w-48 rounded-full transition-all duration-300"
                    style={{
                      border: `2.5px dashed ${faceVisible ? 'rgba(232,89,12,0.7)' : 'rgba(255,255,255,0.15)'}`,
                    }} />
                </div>
                ...
                {faceVisible && previewQuality && qualityConfig[previewQuality] && (
                  <div className="absolute top-2 right-2 rounded-full px-3 py-1 text-[11px] font-bold backdrop-blur-sm"
                    style={{
                      background: qualityConfig[previewQuality].bg,
                      color: qualityConfig[previewQuality].color,
                    }}>
