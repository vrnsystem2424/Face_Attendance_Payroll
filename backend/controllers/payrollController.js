...
// ✅ Future Sundays/Holidays NOT paid in advance
// ✅ Sirf AAJ TAK ke Sundays/Holidays count honge
// ✅ Joining date se pehle ke din skip
// ════════════════════════════════════════════
const calculateEmployeePayroll = async (employee, month, year, settings) => {
  const monthlySalary = employee.monthly_salary || 0;
  const holidays = settings?.holidays || [];
  ...
  // 🎯 AUTOMATIC CUTOFF DAY DETECTION
  // Real IST Date
  const todayIST = moment().tz('Asia/Kolkata');
  const realDay = todayIST.date();
  const realMonth = todayIST.month() + 1;
  const realYear = todayIST.year();

  // Find max date with attendance logged in DB for this month
  let maxAttDay = 0;
  attendanceRecords.forEach(a => {
    if (a.date) {
      const [d] = a.date.split('/').map(Number);
      if (!isNaN(d) && d > maxAttDay) maxAttDay = d;
    }
  });

  let cutoffDay = totalDaysInMonth; // default: full month for past months

  const isCurrentRealMonth = (month === realMonth && year === realYear);
  const isTestingActiveMonth = (maxAttDay > 0 && maxAttDay < totalDaysInMonth);

  if (isCurrentRealMonth) {
    cutoffDay = Math.max(realDay, maxAttDay);
  } else if (isTestingActiveMonth) {
    // In active testing month (e.g., Aug 2026), cutoff is latest day with attendance
    cutoffDay = maxAttDay;
  }
  ...
  // ════════════════════════════════════════
  // 🎯 SUNDAYS & HOLIDAYS (Only Till Cutoff Day)
  // ════════════════════════════════════════
  let sundayCount = 0;        // Total Sundays in full month
  let holidayCount = 0;       // Total Holidays in full month
  let workingDaysCount = 0;   // Total working days in full month
  let weeklyOffPaid = 0;      // ✅ Paid Sundays (till cutoff day)
  let holidaysPaid = 0;       // ✅ Paid Holidays (till cutoff day)
  let skippedBeforeJoining = 0;
  let skippedFuture = 0;
  ...
  for (const dateStr of allDates) {
    ...
    // Full month info counts
    if (holidaySet.has(dateStr)) holidayCount++;
    else if (weeklyOffSetting.includes(dayName)) sundayCount++;
    else workingDaysCount++;

    // ❌ Skip future dates (d > cutoffDay)
    if (d > cutoffDay) {
      skippedFuture++;
      continue;
    }
    ...
    if (joinInfo) {
      ...
      if (currentDate < joinDate) {
        skippedBeforeJoining++;
        continue;
      }
    }

    // ✅ Count for payment ONLY if d <= cutoffDay AND d >= joinDate
  ...
  for (const dateStr of allDates) {
    ...
    // ❌ Skip future dates & dates before joining
    if (d > cutoffDay) continue;

    if (joinInfo) {
      const currentDate = new Date(year, month - 1, d);
      const joinDate = new Date(joinInfo.year, joinInfo.month - 1, joinInfo.day);
      if (currentDate < joinDate) continue;
    }
  ...
  // Absent (weekday, no attendance, no leave) - only passed eligible days
  let absentDays = 0;
  for (const dateStr of allDates) {
    const [d] = dateStr.split('/').map(Number);
    const dayName = getDayName(dateStr);
  ...
  console.log(`\n📊 ${employee.name} (${employee.emp_code}):`);
  console.log(`   📅 Month: ${month}/${year} | Cutoff Day: ${cutoffDay}`);
  console.log(`   ☀️  Weekly Off Paid: ${weeklyOffPaid}/${sundayCount} | Holidays Paid: ${holidaysPaid}/${holidayCount}`);
  console.log(`   👤 Present: ${presentDays} | Sunday Worked: ${sundayWorked} (bonus) | Absent: ${absentDays}`);
  console.log(`   ⏰ Late: ${lateCount}(-${lateLeaveDeduction}d) | HD: ${halfDayCount}(-${halfDayDeduction}d)`);
  console.log(`   📋 Leaves: ${totalLeavesDays}d | Paid: ${paidLeaves} | Unpaid: ${unpaidLeaves}`);
  console.log(`   🎯 Final Payable Days: ${finalPayableDays}/${totalDaysInMonth}`);
  console.log(`   💵 Earned: ₹${earned} | Cut: ₹${cut}`);
  ...
  return {
    ...
    sunday_worked: sundayWorked,
    sunday_count: sundayCount,
    weekly_off_paid: weeklyOffPaid,       // ✅ Fixed: Only passed Sundays
    holiday_count: holidayCount,
    holiday_paid: holidaysPaid,           // ✅ Fixed: Only passed Holidays
    total_days_in_month: totalDaysInMonth,
    skipped_before_joining: skippedBeforeJoining,
