...
//       return res.status(400).json({ success: false, message: 'Valid designation required' });
//     }
...
//     return res.status(200).json({
//       success: true,
//       message: 'Designation update ho gayi',
//       data: employee,
//     });
//   } catch (error) {
//     return res.status(500).json({ success: false, message: 'Server error', error: error.message });
...
//   try {
//     const { leave_approval_manager } = req.body;
...
//     return res.status(200).json({
//       success: true,
//       message: 'Manager update ho gaya',
//       data: employee,
//     });
//   } catch (error) {
//     return res.status(500).json({ success: false, message: 'Server error', error: error.message });
...
//     // Validate
//     if (!worker_type || !['office', 'site'].includes(worker_type)) {
//       return res.status(400).json({ 
//         success: false, 
//         message: 'Valid worker type required (office/site)' 
//       });
//     }

//     // Target employee find karo
//     const targetEmployee = await Employee.findById(req.params.id);
//     if (!targetEmployee) {
...
//     // Company check - dusri company ka employee nahi
//     if (req.employee.role !== 'super_admin') {
//       const myCompanyId = req.employee.company_id?._id?.toString()
//                        || req.employee.company_id?.toString();
//       const targetCompanyId = targetEmployee.company_id?.toString();
...
//     console.log(`✅ Worker Type Updated: ${employee.name} → ${worker_type}`);

//     return res.status(200).json({
//       success: true,
//       message: `Worker type updated to ${worker_type}`,
//       data: employee,
//     });
//   } catch (error) {
...
// ════════════════════════════════════════════
// ✅ UPDATED - APPROVE EMPLOYEE (with joining_date)
// ════════════════════════════════════════════
const approveEmployee = async (req, res) => {
  try {
    const { leave_approval_manager, monthly_salary, joining_date } = req.body;
    ...
    const updateData = {
      ...
      leave_approval_manager: leave_approval_manager || '',
      monthly_salary: salary,
      joining_date: joining_date?.trim() || targetEmployee.joining_date || '',
    };
    ...
    console.log(`✅ Employee Approved: ${employee.name} | Joining: ${updateData.joining_date}`);
...
// UPDATE JOINING DATE
// ════════════════════════════════════════════
const updateJoiningDate = async (req, res) => {
  try {
    ...
    if (joining_date && joining_date.trim() !== '') {
      const parts = joining_date.trim().split('/').map(Number);
      if (
        parts.length !== 3 ||
        parts.some(isNaN) ||
        ...
        return res.status(400).json({
          success: false,
          message: 'Date format sahi nahi. Use: d/m/yyyy (e.g. 14/8/2026)',
        });
      }
    }
    ...
    console.log(`✅ Joining Date Updated: ${employee.name} → ${joining_date}`);

    return res.status(200).json({
      success: true,
      message: 'Joining date update ho gayi',
      data: employee,
    });
  } catch (error) {
